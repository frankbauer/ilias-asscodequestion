...
class codeBlocks implements ArrayAccess {
	...
	private function setJSONEncodedAdditionalData($data) {
		...
		if (!isset($this->additional_data['blocks']) || !is_array($this->additional_data['blocks'])) {
			$this->additional_data['blocks'] = array();
			for ($nr = 0; $nr < 3; $nr++) {
		...
		if (!isset($this->additional_data['domlibs']) || !is_array($this->additional_data['domlibs'])) {
			$this->additional_data['domlibs'] = array();
		...
		if (!isset($this->additional_data['workerlibs']) || !is_array($this->additional_data['workerlibs'])) {
			$this->additional_data['workerlibs'] = array();
		}
	...
	function getRandomSet($setNr) {
		...
		if ($setNr >= 0 && $setNr < count($set))
			$set = $set[$setNr];
		else if ($this->getRandomizerPreviewIndex() >= 0 && $this->getRandomizerPreviewIndex() < count($set))
			$set = $set[$this->getRandomizerPreviewIndex()];
		else if (0 < count($set))
			$set = $set[0];
		else
			$set = NULL;
