    ...
    export default {
        ...
        methods: {
            ...
            finishedExecution(output, infoErrorOutput){
                ...
                let didChangeOutput = false;
                let plygrounds = this.playgrounds;    
                ...
                plygrounds.forEach((block) => {  
                    //console.log('output', output, block, inQuestionEditMode)
                    try {
                        let canvasElement = null;
                        block.obj.init(canvasElement);
                                                
                        if (parseError!=null && typeof calls !== 'undefined'){
                            block.obj.onParseError(initialOutput, parseError)                    
                        }
                        
                        /*canvasElement = $(canvasElement)
                        canvasElement.removeClass('hiddenBlock')*/
                        
                        const result = block.obj.update(output, canvasElement)
                ...
                if (didChangeOutput && initialOutput!=output) {
                    //console.log("changed output");
                    this.output = output.replaceAll('<', '&lt;').replaceAll('>', '&gt;');
                    this.outputHTML = output;
                    this.outputHTML += infoErrorOutput;
                }
            },
            run() {
                let cmp = this.$compilerRegistry.getCompiler(this.compiler);
